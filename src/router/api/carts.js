...
import prod_manager from "../../managers/script.js"
...
router.post('/', async(req,res,next)=> {
    try {
        let response = await carts.addCart(req.body)
        if (response===201) {
...
router.put('/:cid', async(req,res,next)=> {
    try {
        let id = Number(req.params.cid)
        let data = req.body
        let response = await carts.update_cart(id,data)
        if (response===200) {
...
router.put("/:cid/product/:pid/:units", async (req, res, next) => {
    try {
        ...
        let response = await carts.update_cart(cid, id, units);
        if (response === 200) {
            return res.json({ status: 200, message: "cart updated" });
        }
        return res.json({ status: 404, message: "not found" });
...
router.delete('/:cid', async(req,res,next)=> {
    try {
        let id = Number(req.params.cid)
        let response = await carts.delete_cart(id)
        if (response===200) {
...
router.delete("/:cid/product/:pid/:units", async (req, res, next) => {
        ...
        let response = await carts.delete_cart(cid, id, units);
        if (response === 200) {
